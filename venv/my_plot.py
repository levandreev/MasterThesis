...
#     classifier = GaussianNB()
    ...
    y = []
    plot_f1 = []
    with open('amazon_full_all_pos_tags.csv', newline='') as csvfile:
...
df.to_csv('results_amazon_full_allpos_svm1_wstop_unigram.csv', index = False, header = True)

plt.errorbar(N, plot_f1, std_error_array, linestyle='None', marker='.')
plt.savefig('amazon_full_allpos_svm1_wstop_unigram_f1.png')
plt.clf()
plt.errorbar(N, plot_accuracy, std_error_array, linestyle='None', marker='.')
plt.savefig('amazon_full_allpos_svm1_wstop_unigram_acc.png')
