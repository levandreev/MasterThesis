...
#             tfidf_vectorizer = TfidfVectorizer(norm='l2', stop_words='english', ngram_range=(1, 1))
            tfidf_vectorizer = TfidfVectorizer(norm='l2', ngram_range=(2, 2))
...
df.to_csv('results_amazon_pol_allpos_svm1_wstop_bigram.csv', index = False, header = True)

plt.errorbar(N, plot_f1, std_error_array, linestyle='None', marker='.')
plt.savefig('amazon_pol_allpos_svm1_wstop_bigram_f1.png')
plt.clf()
plt.errorbar(N, plot_accuracy, std_error_array, linestyle='None', marker='.')
plt.savefig('amazon_pol_allpos_svm1_wstop_bigram_acc.png')
